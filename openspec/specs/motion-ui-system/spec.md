...
### Requirement: MCP-verified theme from Playful Explorer frames（以 MCP 校验的趣味探索版主题）
系统 MUST 在落地主题前对 **「首页/资产 - 趣味探索版」** 主参考资源调用 Stitch MCP **`get_screen`**（`projects/16236738106685052013/screens/be63fb89830a4850b779356efecb3613`），并以返回内容所体现的色彩层级、圆角与排版气质驱动 **`ColorScheme` / `ThemeData`**；若项目级 `get_project.designTheme` 与 `get_screen` 冲突，MUST 以 **`get_screen`** 为准并在实现处注释说明。

#### Scenario: Theme matches playful explorer focal screen
- **WHEN** 用户冷启动应用进入任意主界面
- **THEN** 表面、主色与强调色角色与上述主参考帧经 MCP 对照后的约定一致（对比度可读），且不再以 **M3 Refined** 或已弃用的其它界面组旧帧为唯一验收依据

### Requirement: Typography per Playful Explorer edition（字体随趣味探索版）
系统 MUST 将标题与正文主字体与 **趣味探索版** 主参考帧（同上 `get_screen`）一致；若该帧使用 **Plus Jakarta Sans** 或其它字体族，MUST 通过 `google_fonts` 或已批准方式加载；失败时 MUST 回退到平台无衬线并记录回退。
...
系统 MUST NOT 使用连续 1px 实线作为主内容区的主要分区手段；MUST 优先使用 **卡片表面、间距与 tonal container**，并与 **趣味探索版** 主参考帧（`get_screen`）的分区方式一致。
...
对任一影响全局主题或壳层导航的 PR，作者 MUST 在描述中列出本 PR 已对照的 **`get_screen` 资源 `name`**；若未对 **趣味探索版** 主帧执行 MCP 读稿，该 PR MUST NOT 合并为「探索版对齐」交付。

#### Scenario: PR cites screen names
- **WHEN** 评审者检查标称为趣味探索版对齐的 UI PR
