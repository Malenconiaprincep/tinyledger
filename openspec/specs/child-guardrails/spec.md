...
### Requirement: Settings matches Playful Explorer edition（设置对齐趣味探索版）
系统 MUST 将设置界面与 Stitch **「设置 - 趣味探索版」** 在**分组标题、列表行样式、开关与入口布局**上一致；**唯一主参考**资源为 `projects/16236738106685052013/screens/776c2074ffea43859bd9c75a8f222fd9`。实现前 MUST **`get_screen`**。MUST 包含 **减弱动效** 开关与 **适龄说明** 入口（可复用既有文案）；家长可见占位 MUST 出现在与主参考帧一致的分组内且不收集敏感个人信息。
